...
abstract class RankingRepository {
  ...
  Future<List<Ranking>> getMyRankings();
...
class RankingRepositoryImpl implements RankingRepository {
  ...
  @override
  Future<void> postRanking(User user, Category category, List<Record> records) async {
  ...
  @override
  Future<List<Ranking>> getMyRankings() async {
    final auth = firebase_auth.FirebaseAuth.instance.currentUser;
    final querySnapshot = await FirebaseFirestore.instance.collection('ranking')
        .orderBy('updatedAt', descending: true)
        .where('userId', whereIn: [auth?.uid])
        .get();
    final rankings = querySnapshot.docs.map((doc) => Ranking.fromJson(doc.data())).toList();
    return rankings;
  }
